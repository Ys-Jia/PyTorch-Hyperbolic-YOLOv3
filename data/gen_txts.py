...
def image_shape(ID, image_dir):
    ...
    if img is not None: return img.shape
...
def process(set_='test', annotation_filename='raw/annotation_val.odgt',
    ...
    output_image_dir = output_dir / "images" # read images from this folder
    output_labels_dir = output_dir / "labels" # read/write labels from/to this folder
    ...
    with open(annotation_filename, 'r') as fanno:
        for raw_anno in fanno.readlines():
            ...
            img_dimension = image_shape(ID, output_image_dir)
            if img_dimension is None: continue

            img_h, img_w, img_c = img_dimension
            assert img_c == 3  # should be a BGR image
            txt_path = output_labels_dir / ('%s.txt' % ID)
            ...
            with open(txt_path.as_posix(), 'w') as ftxt:
                for obj in anno['gtboxes']:
                    ...
                    if 'fbox' in obj.keys():  # full body
                        ...
                        if line:
                            ftxt.write(line)
            jpgs.append('data/%s/%s.jpg' % (output_image_dir, ID))
...
def main():
    ...
    parser.add_argument('--dim', help='input width and height, e.g. 608x608', default="416x416")
    ...
    output_image_dir = output_dir / "images" # create images folder and labels folder
    output_labels_dir = output_dir / "labels"
    output_image_dir.mkdir(exist_ok=True)
    output_labels_dir.mkdir(exist_ok=True)
    for img in output_dir.glob("*.jpg"): # move all images from original unzip folder to images folder
        img.rename(output_image_dir / str(img).split('/')[-1])

    rm_txts(output_labels_dir); rm_txts(output_dir) # remove all label txt files 
    # process('test', 'raw/annotation_val.odgt', output_dir)
    process('train', 'raw/annotation_train.odgt', output_dir)

    with open('../config/crowdhuman-%s.data' % args.dim, 'w') as f:
        f.write("""classes = 2
train   = data/crowdhuman-%s/train.txt
valid   = data/crowdhuman-%s/train.txt
